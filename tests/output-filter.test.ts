...
describe("boostForVerbose", () => {
  it("doubles all head/tail/entry limits (clamped to MAX)", () => {
    ...
    expect(boosted.searchMaxMatchesPerFile).toBe(Math.min(lc.searchMaxMatchesPerFile * 2, 1000));
  });
  ...
  it("re-clamps boosted values to MAX constants (N2 fix)", () => {
    ...
    try {
      ...
      process.env.REASONIX_FILTER_TEST_HEAD = undefined;
      process.env.REASONIX_FILTER_GENERIC_TAIL = undefined;
      process.env.REASONIX_FILTER_FS_MAX_ENTRIES = undefined;
    }
  });
});
